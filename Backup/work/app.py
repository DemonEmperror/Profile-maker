...
# Gemini API Key (Note: Store in environment variables for production)
...
def generate_structured_data(text):
    prompt = f"""
    You are an expert HR resume parser. Convert the following resume text into structured JSON with these fields:
    {{
      "name": "",
    ...
    Instructions:
    - Extract 'name' from the first line or prominent header (e.g., 'Vraj Shah').
    - Identify 'education_training_certifications' from headers like 'Education', 'Certifications', capturing degrees and certificates with start and end dates (e.g., '2005-06').
    - Identify 'total_experience' from headers like 'Total Experience' (e.g., '18 years').
    - Extract 'professional_summary' from headers like 'Professional Summary' or 'Professional Experience Summary'.
    - Identify 'netweb_projects' for projects explicitly mentioning 'NetWeb' or current company roles.
    - Identify 'past_projects' for projects under previous employers or not associated with 'NetWeb'.
    - Extract 'work_experience' from sections like 'Work Experience' or 'Professional Experience', including role, dates, and responsibilities.
    - Extract 'technical_skills' from lists under headers like 'Technical Skill Set', categorizing into web_technologies, scripting_languages, frameworks, databases, web_servers, and tools.
    - Leave fields empty if data is missing, but maintain the JSON structure.
    - Standardize dates to 'YYYY-MM' or 'Month YYYY' format.
...
def sanitize_text(text):
    """Sanitize text for HTML rendering to prevent template injection"""
...
def sanitize_profile_data(profile):
    """Recursively sanitize profile data"""
    if isinstance(profile, str):
        return sanitize_text(profile)
    elif isinstance(profile, dict):
        return {k: sanitize_profile_data(v) for k, v in profile.items()}
    ...
    try:
        with sync_playwright() as p:
            ...
            try:
                ...
                # Set content with proper timeout and wait for loading
                page.set_content(html_string, timeout=120000, wait_until='domcontentloaded')
                page.wait_for_load_state('networkidle', timeout=120000)
                
                # Force print media type for proper styling
...
def render_html_to_docx(profile, output_path, hidden_sections=None):
    ...
    try:
        ...
        if not should_skip_section('summary-section', hidden_sections) and profile.get('professional_summary'):
            ...
            for line in profile['professional_summary'].split('\n'):
                if line.strip():
        ...
        if not should_skip_section('roles-section', hidden_sections) and profile.get('roles_responsibilities'):
            ...
            for line in profile['roles_responsibilities'].split('\n'):
                if line.strip():
                    doc.add_paragraph(line, style='ListBullet')
...
def render_html_to_xlsx(profile, output_path, hidden_sections=None):
    ...
    try:
        ...
        if not should_skip_section('education-section', hidden_sections) and profile.get('education_training_certifications'):
            ...
            row += 1
            for item in profile['education_training_certifications']:
                ws.cell(row=row, column=2).value = f"{item.get('title', '')} ({item.get('start_date', 'N/A')} - {item.get('end_date', 'N/A')})"
                row += 1
        ...
        if not should_skip_section('summary-section', hidden_sections) and profile.get('professional_summary'):
            ...
            ws.cell(row=row, column=2).value = profile['professional_summary'].replace('\n', '; ')
            row += 1
            ...
            if profile.get('netweb_projects'):
                ...
                for project in profile['netweb_projects']:
                    if project.get('title'):
                        ...
                        row += 1
                    if project.get('description'):
                        ws.cell(row=row, column=2).value = f"Description: {project['description'].replace('\n', '; ')}"
                        row += 1
        ...
        if not should_skip_section('roles-section', hidden_sections) and profile.get('roles_responsibilities'):
            ...
            ws.cell(row=row, column=2).value = profile['roles_responsibilities'].replace('\n', '; ')
            row += 1
        ...
        if not should_skip_section('work-experience-section', hidden_sections) and profile.get('work_experience'):
            ...
            row += 1
            for exp in profile['work_experience']:
                ws.cell(row=row, column=2).value = f"{exp.get('role', 'Unknown Role')} ({exp.get('start_date', 'N/A')} - {exp.get('end_date', 'N/A')})"
                row += 1
                if exp.get('responsibilities'):
                    ws.cell(row=row, column=2).value = f"Responsibilities: {exp['responsibilities'].replace('\n', '; ')}"
                    row += 1
...
def index():
    if request.method == 'POST':
        ...
        logging.info(f"Initialized session: profile keys={list(profile.keys())}, hidden_sections=[]")
        try:
...
def submit_from_scratch():
    ...
    profile_data = {
        ...
        "professional_summary": generate_bullet_points(request.form.get('professional_summary', '').strip(), 'professional_summary'),
        "netweb_projects": [
        ...
        "roles_responsibilities": generate_bullet_points(request.form.get('roles_responsibilities', '').strip(), 'roles_responsibilities'),
        "technical_skills": {
        ...
        "work_experience": [
            {
                ...
                "end_date": end.strip(),
                "role": role.strip(),
                "responsibilities": generate_bullet_points(resp.strip(), f'work_experience_responsibilities_{i}')
            }
            for i, (start, end, role, resp) in enumerate(zip(
    ...
    for i, exp in enumerate(data.get('work_experience[]', [])):
        if isinstance(exp, dict):
            ...
            if exp.get('responsibilities', '').strip():
                text_fields[f"work_experience[responsibilities][{i}]"] = {"id": f"work_resp_{i}", "text": exp['responsibilities']}
...
def update_profile():
    ...
    try:
        ...
        hidden_sections = []
    logging.info(f"Parsed hidden_sections: {hidden_sections}")
    ...
    if action == 'save':
        logging.info(f"Rendering display_profile.html with hidden_sections: {hidden_sections}")
        try:
...
def edit_profile():
    ...
    try:
        ...
        flash(f"Template error: {str(e)}. Please ensure profile.html exists.")
        logging.error(f"Template rendering error for profile.html: {str(e)}")
        return redirect('/')

@app.route('/download', methods=['POST'])
def download():
    try:
        # Retrieve profile and hidden sections from session
        profile = session.get('profile', {})
        logging.info(f"Download PDF - Session profile keys: {list(profile.keys()) if profile else 'None'}")
        ...
        # Sanitize profile data to prevent HTML injection
        safe_profile = sanitize_profile_data(profile)

        # Render the HTML template with sanitized profile and hidden sections
        try:
...
def display_profile():
    ...
    if not profile:
        ...
        return redirect('/')
    logging.info(f"Rendering display_profile.html with profile keys={list(profile.keys())}, hidden_sections={hidden_sections}")
    try:
