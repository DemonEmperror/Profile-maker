...
import traceback
import urllib.parse
import bleach  # Added for safe HTML sanitization

# Gemini API Key (Note: Store in environment variables for production)
genai.configure(api_key=os.getenv('GEMINI_API_KEY', "AIzaSyBWHfbDFcuhKwEL-uX0j-SRyUUSidgBhaE"))
...
def has_html_formatting(text):
    """Check if text contains HTML tags used for formatting"""
...
def sanitize_text(text, allow_html=False):
    """Sanitize text for HTML rendering, optionally allowing safe HTML tags"""
    if not isinstance(text, str):
        return text
    if allow_html:
...
def sanitize_profile_data(profile):
    """Recursively sanitize profile data, allowing HTML for specific fields"""
...
def html_to_docx(paragraph, html_text):
    """Convert HTML text to DOCX paragraph with formatting"""
    ...
    try:
        ...
        paragraph.add_run(html_text)  # Fallback to plain text
    ...
    try:
        ...
        if not should_skip_section('work-experience-section', hidden_sections) and profile.get('work_experience'):
            ...
            for exp in profile['work_experience']:
                ...
                row += 1
                if exp.get('responsibilities'):
                    ws.cell(row=row, column=2).value = f"Responsibilities: "
